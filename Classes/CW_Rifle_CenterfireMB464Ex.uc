...
class CW_Rifle_CenterfireMB464Ex extends KFWeap_Rifle_CenterfireMB464;
...
class'Utils'.static.ApplyCustomSkin(self, class'KFGameContent.KFWeap_Rifle_CenterfireMB464');
    ...
    if (GunPerk != none)
    {
        ...
        PutDownTime = 0.25f;
        MinFiringPutDownPct = 0.60f;
        if (!GunPerk.IsSpeedReloadActive())
        {
            ReloadSpeedMod = 0.80f;
        }
        return;
...
simulated function NotifyBeginState()
{
    ...
    else if (InstigatorPerk.Class == class'KFGame.KFPerk_Gunslinger')
    {
        ...
        PutDownTime = 0.25f;
        MinFiringPutDownPct = 0.60f;
        if (!KFPerk_Gunslinger(InstigatorPerk).IsSpeedReloadActive())
        {
            ReloadSpeedMod = 0.80f;
        }
    }
...
DefaultProperties
{
	InstantHitDamageTypes(DEFAULT_FIREMODE)=class'CWP.KFDT_Ballistic_CenterfireMB464Ex'
